...
function OpenCustomUI()
    ...
    -- When sending data to UI, include UI configuration
    SendNUIMessage({
        action = "open",
...
-- Fonction améliorée pour application des modifications avec couleurs exactes
RegisterNUICallback('applyModification', function(data, cb)
    ...
    elseif category == "wheelColor" then
        ...
        -- Cas spécial: si des valeurs RGB sont fournies, les utiliser pour des couleurs personnalisées
        if data.rgb and type(data.rgb) == "table" and #data.rgb == 3 then
            -- Utilisation des valeurs RGB ne serait utile que si le jeu avait une API pour appliquer des couleurs RGB personnalisées
            -- Ce code est inclus pour une compatibilité future si jamais GTA permet les couleurs personnalisées
            -- Pour l'instant, il se contentera d'utiliser l'ID de couleur standard
        end
